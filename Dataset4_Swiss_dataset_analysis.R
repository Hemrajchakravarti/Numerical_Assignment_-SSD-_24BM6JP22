...
numerical_columns = c('Fertility', 'Agriculture', 'Catholic', 'Infant.Mortality')
...
### defining the numerical columns----
categorical_columns = c('Education','Education')
...
par(mfrow=c(2,2))
mult_reg = c('Fertility', 'Agriculture', 'Catholic', 'Infant.Mortality')
mult_reg_data = data[,mult_reg]
fit_mult = lm(Fertility ~ Agriculture + Catholic + Infant.Mortality, data=mult_reg_data)
summary(fit_mult)
plot(fit_mult)
